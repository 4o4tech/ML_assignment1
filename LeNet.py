...
def conv_(img, conv_filter):
    ...
    for i in range(feature_h*feature_w):
        img_matrix[i:] =         img[np.uint16(i/feature_w):np.uint16(i/feature_w+filter_h),np.uint16(i%feature_w):np.uint16(i%feature_w+filter_w)].reshape(filter_w*filter_h)
...
def main():
    ...
    net = ConvNet()
    net.SGD(train_image, train_label, test_image, test_label, 3, 10, 3e-5) 
